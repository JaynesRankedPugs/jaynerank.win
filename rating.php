         ...
         $_POST['result']))
{
    ...
    $result   = $_POST['result'];
    $db       = new PDO(JAYNE_CON . JAYNE_DB, JAYNE_DB_USER, 
                        JAYNE_DB_PASS, $opt);
    ...
    {
        $discord_id = $lobby[$i];
        $do         = $db->prepare("INSERT INTO Main 
                                    (discord_id, name, rating, wins, losses, draws)
                                    VALUES 
                                    (:discordID, :name, :rating, :wins, :losses, :draws)
...
ORDER BY FIELD(discord_id, [{[$ids]}]";
    ...
    if($result=0){
        ...
        $sql = "UPDATE Main 
                SET wins = wins+1, rating = rating+$varwin1 
                WHERE discord_id IN({[$team_ids]})";
        $do  = $db->prepare($sql);
        try {
            $do->execute($team1);
        } catch (PDOException $e) {
        ...
        $sql = "UPDATE Main 
                SET losses = losses+1, rating+$varloss2
                WHERE discord_id IN({[$team_ids]})";
        $do  = $db->prepare($sql);
        try {
            $do->execute($team2);
        ...
        $sql = "UPDATE Main 
                Set losses = losses+1, rating = rating+$varloss1 
                WHERE discord_id IN({[$team_ids]})";
        $do  = $db->prepare($sql);
        try {
            $do->execute($team1);
        ...
        $sql = "UPDATE Main 
                Set wins = wins+1, rating = rating+$varwin2 
                WHERE discord_id IN({[$team_ids]})";
        $do  = $db->prepare($sql);
        try{
            $do->execute($team2);
