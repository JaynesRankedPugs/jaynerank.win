...
function checkBestMustTeam(comb) {
  ...
  var filtered = comb.filter(function(plr) {
    return mustPlayArray.includes(plr['name']);
  }); // filter might be a bottleneck!
...
function findCombinations(slotInfo, teamroles, tProgress, depth) {
  ...
  if (roleInfo['fill'] == roleInfo['slots'].length) {
    ...
    if (curTeam == 'red') { // IN THE PYTHON CODE THIS IS WHERE I MADE A MISTAKE, I PUT BLUE INSTEAD OF RED
      indexes = (arrayRange(roleInfo['prev'] + 1, plrs.length)).filter(function(e) {
        return !slotInfo[curRole]['blueindexes'].includes(e);
      }); // filter might be a bottleneck
    }
  ...
  var sInfo = {
    DPS: {
      ...
      blue: {
        ...
        fill: 0,
        prev: -1
      }, // slots: indexes to fill, fill: number of filled indexes: prev: index to jump back to
      red: {
        slots: [],
        fill: 0,
        prev: -1
